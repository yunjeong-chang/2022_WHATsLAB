    ...
    while True: #cap.isOpened()
        ...
        try :
            ...
            if l_index_shoulder_y < 0 or r_index_shoulder_y < 0 : 
                ...
                if action_1_list[-2] == 0 :
                    start_1 = time.time() #time start

            else :
                action_1_list.append(0)
                if action_1_list[-2] == 1 : 
                    end_1 = time.time() #time end
                    time_1 = round(end_1 - start_1)
                    time_1_list.append(time_1) 
                    if time_1 < 1 :
                        cnt_1 += 1 #1초 미만 실행 횟수 
                    else :
                        cnt_1_warning += 1 #1초 이상 실행 횟수 
        
            # 2 : tilted_head (기울어진 고개)    
            if angle_head_l > 24 or angle_head_r > 24 :
                action_list.append("tilted_head")
                action_2_list.append(2) 
                if action_2_list[-2] == 0 :
                    start_2 = time.time() #time start

            else :
                action_2_list.append(0)
                if action_2_list[-2] == 2 :         
                    end_2 = time.time() #time end
                    time_2 = round(end_2 - start_2)
                    time_2_list.append(time_2) 
                    if time_2 < 2 :
                        cnt_2 += 1 #2초 미만 실행 횟수 
                    else :
                        cnt_2_warning += 1 #2초 이상 실행 횟수
            
            # 3 : hand_on_waist (손 허리 짚기)
            if (l_shoulder_hip_mean_y < l_index[1] and l_index[1] < l_hip[1]-0.04) or (r_shoulder_hip_mean_y < r_index[1] and r_index[1] < r_hip[1]-0.04) :
                if (0 < l_index_hip_x and l_index_hip_x < 0.1) or (0 > r_index_hip_x and r_index_hip_x > -0.1) :
                    ...
                    if action_3_list[-2] == 0 :
                        start_3 = time.time() #time start
            else :
                action_3_list.append(0)
                if action_3_list[-2] == 3 :
                    end_3 = time.time() #time end
                    time_3 = round(end_3 - start_3)
                    time_3_list.append(time_3) 
                    if time_3 < 2 :
                        cnt_3 += 1 #2초 미만 실행 횟수 
                    else :
                        cnt_3_warning += 1 #2초 이상 실행 횟수 
            ...
            if (l_shoulder[1] < l_index[1] and l_index[1] < l_shoulder_hip_mean_y) or (r_shoulder[1] < r_index[1] and r_index[1] < r_shoulder_hip_mean_y) :
                if (l_index_hip_x < 0) or (r_index_hip_x > 0) : 
                    ...
                    if action_4_list[-2] == 0 :
                        start_4 = time.time() #time start
            else :
                action_4_list.append(0)
                if action_4_list[-2] == 4 :
                    end_4 = time.time() #time end
                    time_4 = round(end_4 - start_4)
                    time_4_list.append(time_4) 
                    if time_4 < 2 :
                        cnt_4 += 1 #2초 미만 실행 횟수
                    else :
                        cnt_4_warning += 1 #2초 이상 실행 횟수 
                
            # 5 : tilted_body (기울어진 상체, 허리 스트레칭)
            if angle_body_l < 155 or angle_body_r < 155 : 
                action_list.append("tilted_body")
                action_5_list.append(5) 
                if action_5_list[-2] == 0 :
                        start_5 = time.time() #time start  
            else :
                action_5_list.append(0)
                if action_5_list[-2] == 5 :
                    cnt_5 += 1 
                    end_5 = time.time() #time end
                    time_5 = round(end_5 - start_5)
                    time_5_list.append(time_5) 
                    if time_5 < 1 :
                        cnt_5 += 1 #1초 미만 실행 횟수
                    else :
                        cnt_5_warning += 1 #1초 이상 실행 횟수 
                
            # 6 : normal_posture (배 위에 양 손 모아 공손 or 차렷) 
            if (((l_shoulder_hip_mean_y < l_index[1] and l_index[1] < l_hip[1]+0.02) and (r_shoulder_hip_mean_y < r_index[1] and r_index[1] < r_hip[1]+0.02)) and ((l_index_hip_x < 0) and (r_index_hip_x > 0)))\
                or (((l_hip[1] < l_index[1]) and (r_hip[1] < r_index[1])) and ((0.01 < l_index_hip_x and l_index_hip_x < 0.2) and (-0.01 > r_index_hip_x and r_index_hip_x > -0.2))) :
                action_list.append("normal_posture")
                action_6_list.append(6) 
                if action_6_list[-2] == 0 :
                        start_6 = time.time() #time start
            
            else :
                action_6_list.append(0)
                if action_6_list[-2] == 6 :
                    end_6 = time.time() #time end
                    time_6 = round(end_6 - start_6)
                    if time_6 < 2 :
                        cnt_6 += 1 #pose change
                    else :
                        cnt_6_warning += 1 
                        time_6_list.append(time_6) 
            ...
            #putText(mask, "l_index_hip_x: {} ".format(str(l_index_hip_x)), (20,250))
            #putText(mask, "r_index_hip_x: {} ".format(str(r_index_hip_x)), (20,300))
...
print('hand_to_face : ', cnt_1_warning)
print('tilted_head: ', cnt_2_warning)
print('hand_on_waist: ', cnt_3_warning)
print('hand_on_chest: ', cnt_4_warning)
print('tilted_body: ', cnt_5_warning)
print('normal_posture: ', cnt_6_warning, time_6_list)

#전송할 목록
#1.손 얼굴 높이   : 해당 동작을 1초 이상 취한 횟수 => cnt_1_warning
#2.기울어진 고개  : 해당 동작을 2초 이상 취한 횟수 => cnt_2_warning
#3.손 허리        : 해당 동작을 2초 이상 취한 횟수 => cnt_3_warning
#4.손 팔짱        : 해당 동작을 2초 이상 취한 횟수 => cnt_4_warning
#5.기울어진 상체  : 해당 동작을 1초 이상 취한 횟수 => cnt_5_warning
#6.정자세(배 위에 양 손 모아 공손,손 차렷) : 해당 동작을 유지하지 못한 횟수 => cnt_6_warning, 해당 동작을 유지한 시간 => time_6_list
